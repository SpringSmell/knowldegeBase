...
import com.yidingliu.dev.knowledgebase.tools.AnalyzeView;
import com.yidingliu.dev.knowledgebase.tools.DoubleSineGen;
import com.yidingliu.dev.knowledgebase.tools.RealDoubleFFT;
import com.yidingliu.dev.knowledgebase.tools.SelectorText;
...
public class AnalyzeActivity extends Activity
		implements OnLongClickListener, OnClickListener, AnalyzeView.Ready, OnSharedPreferenceChangeListener {
	...
	/**
	 * Run processClick() for views, transferring the state in the textView to
	 * our internal state, then begin sampling and processing audio data
	 */
	...
	private void showInstructions() {
		...
		tv.setText(Html.fromHtml("说明"));
		new AlertDialog.Builder(this).setTitle("说明标题").setView(tv)
				.setNegativeButton("消失", null).create().show();
	}
	...
	private void refreshCursorLabel() {
		...
		((TextView) findViewById(R.id.freq_db)).setText(Math.round(f) + "hz\n" + Math.round(graphView.getY()) + "db");
	}
	...
	public class Looper extends Thread {
		...
		public Looper() {
			...
			record = new AudioRecord(AGC_OFF, sampleRate, AudioFormat.CHANNEL_IN_MONO, AudioFormat.ENCODING_PCM_16BIT,
					minBytes);
	 ...
	 * @param cmd
	 *            The command to run for each view
	 * @param select
	 *            The tag value that must match. Null implies all views
	 */
